 ...
 * Init function called) b4     (not used) x  : Should be set to 0 b3 0  : Ready
 * or Busy (No Listen mode ongoing) 1  : Listen (Peripheral in Address Listen
 * Mode) b2     Intrinsic process state 0  : Ready 1  : Busy (Peripheral busy
 * with some configuration or internal operations) b1     Rx state 0  : Ready
 * (no Rx operation ongoing) 1  : Busy (Rx operation ongoing) b0     Tx state 0
 * : Ready (no Tx operation ongoing) 1  : Busy (Tx operation ongoing)
 * @{
 */
typedef enum {
...
/** @brief  Clears the I2C pending flags which are cleared by writing 0 in a
 * specific bit.
 * @param  __HANDLE__ specifies the I2C Handle.
 ...
 * @param  __HANDLE__ specifies the I2C Handle.
 *         This parameter can be I2C where x: 1, 2, or 3 to select the I2C
 * peripheral.
 * @retval None
 */
...
#define I2C_RISE_TIME(__FREQRANGE__, __SPEED__)                                \
  (((__SPEED__) <= 100000U) ? ((__FREQRANGE__) + 1U)                           \
...
/** @defgroup I2C_IS_RTC_Definitions I2C Private macros to check input
 * parameters
 * @{
 */
