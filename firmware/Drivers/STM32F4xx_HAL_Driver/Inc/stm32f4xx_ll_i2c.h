...
typedef struct {
  uint32_t PeripheralMode; /*!< Specifies the peripheral mode.
                              ...
                              unitary function @ref LL_I2C_SetMode(). */
  ...
  uint32_t
      ClockSpeed; /*!< Specifies the clock frequency.
                     ...
                     function @ref LL_I2C_SetClockPeriod() or @ref
                     LL_I2C_SetDutyCycle() or @ref LL_I2C_SetClockSpeedMode() or
  ...
  uint32_t
      DutyCycle; /*!< Specifies the I2C fast mode duty cycle.
                      ...
                      This feature can be modified afterwards using unitary
                    function @ref LL_I2C_SetDutyCycle(). */
  ...
  uint32_t AnalogFilter; /*!< Enables or disables analog noise filter.
                            ...
                            I2C_LL_EC_ANALOGFILTER_SELECTION
                              ...
                              This feature can be modified afterwards using
                            unitary functions @ref LL_I2C_EnableAnalogFilter()
                            or LL_I2C_DisableAnalogFilter(). */
  ...
  uint32_t DigitalFilter; /*!< Configures the digital noise filter.
                             ...
                             0x00 and Max_Data = 0x0F
                               ...
                               This feature can be modified afterwards using
                             unitary function @ref LL_I2C_SetDigitalFilter(). */
  ...
  uint32_t OwnAddress1; /*!< Specifies the device own address 1.
                           ...
                           0x00 and Max_Data = 0x3FF
                             ...
                             This feature can be modified afterwards using
                           unitary function @ref LL_I2C_SetOwnAddress1(). */

  uint32_t
      TypeAcknowledge; /*!< Specifies the ACKnowledge or Non ACKnowledge
                          condition after the address receive match code or next
                          received byte. This parameter can be a value of @ref
                          I2C_LL_EC_I2C_ACKNOWLEDGE

                            This feature can be modified afterwards using
                          unitary function @ref LL_I2C_AcknowledgeNextData(). */
...
#define LL_I2C_SR1_ARLO                                                        \
  I2C_SR1_ARLO /*!< Arbitration lost                          */
#define LL_I2C_SR1_AF                                                          \
  I2C_SR1_AF /*!< Acknowledge failure flag                  */
...
#define LL_I2C_SR1_SMALERT                                                     \
  I2C_ISR_SMALERT /*!< SMBus alert (SMBus mode)                  */
#define LL_I2C_SR2_MSL                                                         \
  I2C_SR2_MSL /*!< Master/Slave flag                         */
#define LL_I2C_SR2_BUSY                                                        \
  I2C_SR2_BUSY /*!< Bus busy flag                             */
#define LL_I2C_SR2_TRA                                                         \
  I2C_SR2_TRA /*!< Transmitter/receiver direction            */
#define LL_I2C_SR2_GENCALL                                                     \
  I2C_SR2_GENCALL /*!< General call address (Slave mode)         */
#define LL_I2C_SR2_SMBDEFAULT                                                  \
  I2C_SR2_SMBDEFAULT /*!< SMBus Device default address (Slave mode) */
#define LL_I2C_SR2_SMBHOST                                                     \
  I2C_SR2_SMBHOST /*!< SMBus Host address (Slave mode)           */
...
#define LL_I2C_ANALOGFILTER_DISABLE                                            \
  I2C_FLTR_ANOFF /*!< Analog filter is disabled.*/
/**
 * @}
 */
...
#define LL_I2C_OWNADDRESS1_7BIT                                                \
  0x00004000U /*!< Own address 1 is a 7-bit address.   */
#define LL_I2C_OWNADDRESS1_10BIT                                               \
  (uint32_t)(I2C_OAR1_ADDMODE |                                                \
             0x00004000U) /*!< Own address 1 is a 10-bit address.  */
/**
 * @}
 */
...
#define LL_I2C_DUTYCYCLE_2                                                     \
  0x00000000U /*!< I2C fast mode Tlow/Thigh = 2        */
#define LL_I2C_DUTYCYCLE_16_9                                                  \
  I2C_CCR_DUTY /*!< I2C fast mode Tlow/Thigh = 16/9     */
/**
 * @}
 */
...
#define LL_I2C_CLOCK_SPEED_FAST_MODE                                           \
  I2C_CCR_FS /*!< Master clock speed range is fast mode     */
/**
 * @}
 */
...
#define LL_I2C_MODE_SMBUS_DEVICE                                               \
  I2C_CR1_SMBUS /*!< SMBus Device default mode (Default address not            \
                   acknowledge) */
#define LL_I2C_MODE_SMBUS_DEVICE_ARP                                           \
  (uint32_t)(I2C_CR1_SMBUS |                                                   \
             I2C_CR1_ENARP) /*!< SMBus Device Default address acknowledge */
/**
 * @}
 */
 ...
 */
#define LL_I2C_ACK                                                             \
  I2C_CR1_ACK /*!< ACK is sent after current received byte.                    \
               */
#define LL_I2C_NACK                                                            \
  0x00000000U /*!< NACK is sent after current received                         \
                 byte.*/
/**
 * @}
 */
 ...
 * @param  __FREQRANGE__ This parameter must be a value of peripheral clock (in
 * Mhz).
...
/**
 * @brief  Compute Speed clock range to a Clock Control Register (I2C_CCR_CCR)
 * value.
 * @param  __PCLK__ This parameter must be a value of peripheral clock (in Hz).
 * @param  __SPEED__ This parameter must be a value lower than 400kHz (in Hz).
 ...
 * @retval Value between Min_Data=0x004 and Max_Data=0xFFF, except in FAST DUTY
...
#define __LL_I2C_SPEED_TO_CCR(__PCLK__, __SPEED__, __DUTYCYCLE__)              \
  (uint32_t)(((__SPEED__) <= LL_I2C_MAX_SPEED_STANDARD)                        \
                 ...
                 : (__LL_I2C_SPEED_FAST_TO_CCR((__PCLK__), (__SPEED__),        \
                                               (__DUTYCYCLE__))))

/**
 * @brief  Compute Speed Standard clock range to a Clock Control Register
 * (I2C_CCR_CCR) value.
 * @param  __PCLK__ This parameter must be a value of peripheral clock (in Hz).
...
/**
 * @brief  Compute Speed Fast clock range to a Clock Control Register
 * (I2C_CCR_CCR) value.
 * @param  __PCLK__ This parameter must be a value of peripheral clock (in Hz).
 * @param  __SPEED__ This parameter must be a value between Min_Data=100Khz and
 ...
 * @param  __ADDRESS__ This parameter must be a value of a 10-Bits slave
 * address.
...
#define __LL_I2C_10BIT_HEADER_WRITE(__ADDRESS__)                               \
  ((uint8_t)((                                                                 \
      uint16_t)((uint16_t)(((uint16_t)((__ADDRESS__) & (uint16_t)(0x0300))) >> \
                           7) |                                                \
                (uint16_t)(0xF0))))
 ...
 * analog filter. The filters can only be programmed when the I2C is disabled
 * (PE = 0).
 ...
 * filtering capability up to 15*TPCLK1) This parameter is used to configure the
 * digital noise filter on SDA and SCL input. The digital filter will suppress
 * the spikes with a length of up to DNF[3:0]*TPCLK1.
 * @retval None
 */
 ...
 * @note   If the analog filter is also enabled, the digital filter is added to
 * analog filter. This filter can only be programmed when the I2C is disabled
 * (PE = 0).
...
__STATIC_INLINE void LL_I2C_SetOwnAddress1(I2C_TypeDef *I2Cx,
  ...
  MODIFY_REG(I2Cx->OAR1,
             I2C_OAR1_ADD0 | I2C_OAR1_ADD1_7 | I2C_OAR1_ADD8_9 |
                 I2C_OAR1_ADDMODE,
 ...
 * @param  I2Cx I2C Instance.
 * @param  OwnAddress2 This parameter must be a value between Min_Data=0 and
 * Max_Data=0x7F.
 * @retval None
 */
 ...
 * @param  I2Cx I2C Instance.
 * @param  RiseTime This parameter must be a value between Min_Data=0x02 and
 * Max_Data=0x3F.
 * @retval None
 */
 ...
 * @param  I2Cx I2C Instance.
 * @param  ClockPeriod This parameter must be a value between Min_Data=0x004 and
 * Max_Data=0xFFF, except in FAST DUTY mode where Min_Data=0x001.
 * @retval None
 */
...
__STATIC_INLINE void LL_I2C_ConfigSpeed(I2C_TypeDef *I2Cx, uint32_t PeriphClock,
  ...
  if (ClockSpeed > LL_I2C_MAX_SPEED_STANDARD) {
    /* Set Speed mode at fast and duty cycle for Clock Speed request in fast
     * clock range */
    clockconfig =
    ...
    /* Set Speed mode at standard for Clock Speed request in standard clock
     * range */
 ...
 *
 * @note   Any of these events will generate interrupt if Buffer interrupts are
 * enabled too(using unitary function @ref LL_I2C_EnableIT_BUF()) : Receive
 ...
 * @note   Any of these Buffer events will generate interrupt if Events
 * interrupts are enabled too(using unitary function @ref LL_I2C_EnableIT_EVT())
 * : Receive buffer not empty (RXNE) Transmit buffer empty (TXE)
...
/**
 * @brief  Indicate the status of Address sent (master mode) or Address matched
 * flag (slave mode).
 * @note   RESET: Clear default value.
 *         SET: When the address is fully sent (master mode) or when the
 * received slave address matched with one of the enabled slave address (slave
 * mode).
 ...
 *         SET: When an acknowledge failure is received after a byte
 * transmission.
 ...
 * @note   This status is cleared by hardware after a STOP condition or repeated
 * START condition.
...
/**
 * @brief  Indicate the status of SMBus Device default address reception (Slave
 * mode).
 * @note   Macro IS_SMBUS_ALL_INSTANCE(I2Cx) can be used to check whether or not
 *         SMBus feature is supported by the I2Cx Instance.
...
LL_I2C_IsActiveSMBusFlag_SMBDEFAULT(I2C_TypeDef *I2Cx) {
 ...
 * @note   In that case, the ACK bit controls the (N)ACK of the next byte
 * received or the PEC bit indicates that the next byte in shift register is a
 * PEC.
 ...
 * @note   In that case, the ACK bit controls the (N)ACK of the current byte
 * received or the PEC bit indicates that the current byte in shift register is
 * a PEC.
...
/**
 * @brief  Enable transfer or internal comparison of the SMBus Packet Error byte
 * (transmission or reception mode).
 * @note   Macro IS_SMBUS_ALL_INSTANCE(I2Cx) can be used to check whether or not
 *         SMBus feature is supported by the I2Cx Instance.
 * @note   This feature is cleared by hardware when the PEC byte is transferred
 * or compared, or by a START or STOP condition, it is also cleared by software.
...
/**
 * @brief  Disable transfer or internal comparison of the SMBus Packet Error
...
/**
 * @brief  Check if the SMBus Packet Error byte transfer or internal comparison
 * is requested or not.
 * @note   Macro IS_SMBUS_ALL_INSTANCE(I2Cx) can be used to check whether or not
 *         SMBus feature is supported by the I2Cx Instance.
